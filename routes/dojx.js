...
exports.dosay = function(req, res) {
	...
	var who = req.session.user && req.session.user.name;
	...
	User.get(who, function(err, user) {
		...
		Post.getOnly(username, id, function(err, post) {
			if(err) {
				return res.status(400).send({err: "该微言不存在！"});
			}
...
// 微文点赞
exports.doessay = function(req, res) {
	var id = ObjectID(req.query.id);
	var username = req.query.username;
	var who = req.session.user && req.session.user.name;
	...
	User.get(who, function(err, user) {
		...
		// 判断被点赞微文是否存在
		Article.getOnly(username, id, function(err, doc) {
			if (err) {
				return res.status(400).send({err: "该微文不存在！"});
			}

			// 判断用户是否点过赞
			doc.whoups = doc.whoups || [];
			if (doc.whoups.indexOf(who) !== -1) {
				return res.status(400).send({err:'亲，您已经点过了！', flag: 4});
			}

			Article.update(username, id, who, function(err) {
				if (err) {
					res.status(400).send({err: err});
				} else {
					res.status(200).send({suc: 'success!'});
